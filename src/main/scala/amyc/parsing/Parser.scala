...
object Parser extends Pipeline[Iterator[Token], Program] with Parsers {
  ...
  lazy val simplePattern: Syntax[Pattern] =
    unitPattern | literalPattern | wildPattern

  lazy val classPattern: Syntax[Pattern] =
    (identifier ~ opt(
      opt(delimiter(".") ~>~ identifier) ~<~ delimiter("(") ~ repsep(
        pattern,
        ","
      ) ~<~ delimiter(")")
    )).map {

      case ident1 ~ Some(Some(ident2) ~ seq) =>
        CaseClassPattern(QualifiedName(Some(ident1), ident2), seq.toList)

      case ident1 ~ Some(None ~ seq) =>
        CaseClassPattern(QualifiedName(None, ident1), seq.toList)
  ...
  lazy val unitPattern: Syntax[Pattern] =
    (delimiter("(") ~ delimiter(")")).map { case _ =>
    ...
    (prior2 ~ opt(";" ~>~ expr)).map {
      ...
      case expr1 ~ Some(expr2) =>
        Sequence(
          expr1,
          expr2
        ) // expr.foldLeft(tail.head)((e1, e2) => Sequence(e1, e2))

    }

  lazy val matchStatement: Syntax[Expr] =
    ((prior3to9 | ifStatement) ~ many(
      kw("match") ~>~ delimiter("{") ~>~ many1(matchCase) ~<~ delimiter("}")
    )).map { case scrut ~ matches =>
      matches.toList match

        case head :: next =>
          next.foldLeft(Match(scrut, head.toList))((z, x) => Match(z, x.toList))
...
//e idea is to put expr of different priorities in different val, and surrounding ones with high priority with low priority. So simpleExpr is the one with highest priority, then the one follows is Unary, so u want sth like "Unary = opt(op(!) | Op(-))~simpleExpr"
  ...
  lazy val simpleExpr: Syntax[Expr] =
    otherLiteral.up[
      Expr
    ] | unitOrParExpr | variableOrCall | throwError // | ifStatement | varDef
